        ...
        try {
            ...
            if (fromTheLoop) { // Only process if the loop told us to
                ...
                if (rateLimitArr.length >= projectionFidelity) {
                    ...
                    log(`currentRatePerSecond: ${currentRatePerSecond}, target: ${targetRatePerSecond}, currentThreadRate: ${currentThreadRate}`)
                    var secondsInRate = opts.rateLimitSeconds
                    var currentSeconds = projDataSet.length * (rateLimitInterval / 1000)
                    var millisecondsSaved = ((opts.rateLimit / opts.rateLimitSeconds) - (totalRealCount / currentSeconds)) * 1000
                    // log(`currentSeconds: ${currentSeconds}`)
                    var futureSeconds = secondsInRate - currentSeconds
                    // log(`futureSeconds: ${futureSeconds}`)
                    var projectedFutureCount = futureSeconds * currentRatePerSecond
                    // log(`projectedFutureCount: ${projectedFutureCount}`)
                    var projectedCount = totalRealCount + projectedFutureCount
                    log(`projectedCount: ${projectedCount}`)
                    var projectedRate = projectedCount / secondsInRate
                    log(`projectedRatePerSecond: ${projectedRate}`)
                    var neededChange = targetRatePerSecond - projectedRate
                    log(`neededChange: ${neededChange}`)

                    // log(`avgThreads: ${avgThreads}`)
                    // var avgThreadRate = projRate.count / avgThreads
                    // log(`avgThreadRate: ${avgThreadRate}`)
                    // var projectedFutureSegments = ((projRate.count / projectionFidelity) * (opts.rateFidelity - projectionFidelity))
                    // log(`Projected Next ${(opts.rateFidelity - projectionFidelity)} Segments: ${projectedFutureSegments}`)
                    // var projection = projRate.count + projectedFutureSegments
                    // log(`projection: ${projection}`)
                    // var neededChange = opts.rateLimit - projection
                    // log(`neededChange: ${neededChange}`)
                    ...
                    log(`minimumThreadTime: ${minimumThreadTime}`)
                    log(`targetThreads: ${targetThreads}`)
                    log(`currentThreads: ${threadCount}`)
